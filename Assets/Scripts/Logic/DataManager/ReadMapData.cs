...
public class ReadMapData : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            persistentData = Resources.Load<PersistentData>("PersistentData");
        }
    ...
    void SetUpBlock(GameObject block, int row, int col, string cell) 
    {
        ...
        if (block.TryGetComponent<RectTransform>(out var rt))
        {
            ...
            rt.anchoredPosition = GetBlockPosition(row, col);
        }
        ...
        if(cell != "0")
        {
            ...
            swipeController.SetPersistentData(persistentData);
        }
    }
    public void ReadData(int currentLevel)
    {
